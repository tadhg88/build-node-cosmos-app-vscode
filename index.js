...
var config = {
  connectionString:
    "AccountEndpoint=https://tadhg88-cosmosdb-test.documents.azure.com:443/;AccountKey=Samw4seqXpOe6MFy64gvAwB9u5Y0u1a8kFb864XLAxfQk6ldHWd0YfvvxjH7dp7V9vaMLPH1PyJ6j4lLV4H4Og==",
  database: "SchoolDB",
...
async function addStudent(student) {
  ...
  isOK(statusCode) &&
    process.stdout.write(`Added student with id: ${item.id}\n`);
}

async function updateStudent(student) {
  const { item, statusCode } = await containerdata.upsert(student).catch();
  isOK(statusCode) &&
    process.stdout.write(`Updated student with id: ${item.id}\n`);
}

async function deleteStudent(student) {
  const { item, statusCode } = await containerref
    .item(student.id, student.StudentNumber)
    .delete()
    .catch();
  isOK(statusCode) &&
    process.stdout.write(`Deleted student with id: ${item.id}\n`);
}

// This function fetches a student document, given the id and the studentNumber (the partition key)
async function getStudent(ID, studentNumber) {
  const { resource, statusCode } = await containerref
    .item(ID, studentNumber)
    .read()
    .catch();
  if (isOK(statusCode)) {
    process.stdout.write(
      `Student data: ${resource.StudentNumber}: ${resource.Forename}, ${resource.Lastname}\n`
    );
...
async function queryStudents(courseName) {
  const studentquery = {
    query:
      "SELECT s.StudentNumber, s.Forename, s.Lastname, c.Course, c.Grade \
